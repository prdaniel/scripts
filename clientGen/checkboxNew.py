...
def infoMessage():
    tkMessageBox.showinfo(infoList[jeng].replace('_',' ').upper(),fileList[jeng])
...
##Sets the variables for the while loop##
...
while length >= 0 :
    ...
    box = dicf[string+str(length)]
    Button(master, text = "Info", command =box).grid(row=rows, column=1, sticky=W)
    #While Loop Counters#
    length = length - 1
    rows+=1
    varbs+=1

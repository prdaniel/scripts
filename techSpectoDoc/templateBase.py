...
client = raw_input("Enter Client Name for Document:  ")
appdir = raw_input("Enter Application Root Directory\n(Hit Enter for Default: E:/Endeca/apps/):  ")
if appdir == "":
    appdir = "E:/Endeca/apps/"
appname = raw_input("Enter the Endeca app name\n(Example: E:/Endeca/apps/pbs/ type pbs): ")
app = appdir+appname+"/"
endecadir = appdir.split('apps'.lower())[0]
tools = raw_input("Enter ToolsAndFrameworks version\n(Hit Enter for Default: 11.0.0): ")
if tools == "":
    tools = "11.0.0"
document = Document()
...
p.add_run("There are (4) main data types that make up the "+client+" site indices:")
...
p.add_run("There are (3) data files used to update the "+client+" site through the partial update process.")
...
p = document.add_paragraph("")
p.add_run("\nData Pre-processing\n").bold = True
p.add_run("All data pre-processing is performed before the data is loaded into the "+appdir+"pbsconnect/data/ektron/full/ directory. Before the data is fed into the pipeline, Thanx has implemented several python scripts to clean and check the data. "+app+"")
...
p = document.add_paragraph("")
p.add_run("\nLanguage Configuration").bold = True
...
document.add_paragraph('The strategy is based on a search interface with members such as Name, Category, and Description, in that order.  The order is significant because a match on the first member ranks more highly than a cross-field match or match on the second or third member.', style='ListBullet')
p.add_run("The table below defines the relevance ranking modules to be configured for the search interface. A static module is normally recommended as the final module, in this instance, "+client+" has added the date_sort_sixty_days as the static module in the 'All' search interface, and searchrank as the static module in the 'search_rank_topic' search interface.")
...
row_cells[1].text = "NumFields has been added to match the current search strategy currently in place on "+client+". Numfields ranks records based on the number of fields that match a query completely. "
row_cells = table.add_row().cells
...
row_cells = table.add_row().cells
document.add_heading("Merchandising Workbench Configuration", 1)
...
p.add_run("The pipeline defines the import, conversion, and mapping of source data to Endeca IAP Properties and Dimensions.  "+client+" will have one pipeline indexing the product catalog. The pipeline is generated using the Endeca "+tools+" Product Catalog Integration deployment template. No changes to default pipeline layout were made.\n\n")
...
p.add_run("\nThe "+client+" implementation encompasses the server environments below.  Please refer to section 17.7: \"Migration of Configuration Files between Environments\" for details on migration of the Endeca configuration between environments.\n\nOverall design of the cluster will be the same for the Dev, Test and Production environments. The diagram below illustrates the overall design, and the installed Endeca software on each server.")
...
p.add_run("\nThe dev environment is a single server environment providing the Data Foundry services, and MDEX Engine to serve query requests to the web/application servers in the "+client+" dev environment, the Endeca Workbench, and Logging / Report Generator server.  The server hostname is Endeca-DEV-01.\n\n")
document.add_paragraph(
    'Endeca-DEV-01', style='ListBullet'
)
p.add_run("Test Environment\n").bold = True
p.add_run("The test environment is a single server environment providing the Data Foundry services, and MDEX Engine to serve query requests to the web/application servers in the "+client+" test environment, the Endeca Workbench, and Logging / Report Generator server.  The server hostname is Endeca-TEST-01.\n")
document.add_paragraph(
    'Endeca-TEST-01', style='ListBullet'
)

p.add_run("\nProduction Environment\n").bold = True
p.add_run("The production environment is a single (3) server environment providing the Data Foundry services, and MDEX Engine to serve query requests to the web/application servers in the "+client+" test environment, the Endeca Workbench, and Logging / Report Generator server.   The indexing server hostname is ENDECA-PROD-01.HQ.CORP.PBS.ORG.")
document.add_paragraph(
...
p = document.add_paragraph("")
p.add_run("Software Installation\n").bold = True
...
p.add_run("Most of the Endeca software should be installed and all Endeca processes run as the \"Administrator\" user or equivalent. There may be permission issues during baseline updates if files are owned by different users.\n\nEach of the "+client+" servers running an Endeca process have user accounts created for the Thanx Media implementation team to provide access to all machines in all the Endeca environments.  Upon logging into that server, the Thanx Media users should have the same rights as the \"Administrator\" user to perform implementation related tasks.\n\nThe Thanx Media team has been given the \"sys-endeca\" user account with Administrator rights.\n\n")

p.add_run("Application Installation and Directory Structure\n").bold = True
p.add_run("Each "+client+" application consists of all configuration XML and supporting documents, custom Control Scripts (defined below), shell scripts and, following data updates, MDEX index files and necessary directory structure to perform all implementation-specific tasks including data forge/indexing, MDEX Engine query responses, logging and report generation. \n\nThe "+client+" application will be deployed by Thanx to the following directory:")

table = document.add_table(rows=1, cols=1)
hdr_cells = table.rows[0].cells
hdr_cells[0].text = "Directory"
row_cells = table.add_row().cells
row_cells[0].text = ""+appdir+"pbs/"

p = document.add_paragraph("")
p.add_run("\nThe Endeca Deployment Template is a collection of operational components that provides a starting point for development and application deployment.  This template includes the complete directory structure required for deployment, including EAC scripts, configuration files, and batch files or shell scripts that wrap common script functionality.\n\nThe current Endeca Deployment Template is version PCI "+tools+" and will be used for the "+client+" applications.")

table = document.add_table(rows=1, cols=1)
hdr_cells = table.rows[0].cells
hdr_cells[0].text = "Deployment Script Location"
row_cells = table.add_row().cells
row_cells[0].text = ""+endecadir+"ToolsAndFrameworks/"+tools+"/deployment_template/bin/"

p = document.add_paragraph("")
p.add_run("From the execution of the Endeca Deployment template's deploy.bat script, the full directory structure used by the application on the primary server is: \n\n")
...
hdr_cells[0].text = "Directory"
row_cells = table.add_row().cells
row_cells[0].text = ""+endecadir+"ToolsAndFrameworks/"+tools+"/server/workspace/conf/"
...
p.add_run(""+endecadir+"ToolsAndFrameworks/"+tools+"/reference/pbs-service/WEB-INF/assembler.properties. All variables will need to be updated to match the configuration as noted in the above sections. For mdex host and port, please reference the load balancer port and host if configured. ")
...
p = document.add_paragraph("")
p.add_run("When all of the values are commented out, all properties will return.\n\nFinally, add the service context file by creating a file called docen.xml in the "+endecadir+"ToolsAndFrameworks/"+tools+"/erver/workspace/conf/Standalone/localhost directory. The file will contain the path from the url and the service path location as seen below: ")
...
p.add_run("Development Environment\n").bold = True
p.add_run("Application Development will occur in the development environment and deployed directly to that environment.\n\nDevelopment occurs with the Developer Studio tool which should be installed on each developer's computer.  To facilitate ease of development, we recommended using "+client+"'s current version control software (e.g. Visual SourceSafe, Subversion).\n\n")
...
hdr_cells[0].text = "Directory"
row_cells = table.add_row().cells
row_cells[0].text = ""+app+"control/promote_content.bat"

p = document.add_paragraph("")
p.add_run("\nOr from within the Workbench admin console:")
